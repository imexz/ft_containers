...
// TESTS MAP
	...
	{
		test.insert(ft::pair<int, int>(rand() % 100000, i));
		cout << test.size() << endl;
	}
	for (size_t i = 0; i < 1000000; i++)
	{
		test.erase(rand() % 1000000);
		if (i % 3 == 0)
			test.insert(ft::pair<int, int>(rand() % 10000, i));
		if (i  % 100000 == 0)
			test.clear();
	...
	{
		ft::map<int, int>::iterator g = test.find(rand() % 10000);
		if (g != test.end()) {
			cout << g->first << endl;
		}
